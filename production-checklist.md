...
![](.gitbook/assets/screenshot-2021-09-08-at-08.33.28.png)
...
## Article details checks and edits 

This section will explain which items you need to check and what you need to look out for. In order to edit these sections 

Raising a typesetter query if this is something you are unable to do - screenshots of how to do this. Editing the text.
...
* Explain that the publication date will be validated when you check the XML. 
...
The Data availability statement should ideally be at least one sentence and should end in a full-stop. Generated datasets mentioned in this statement should be included in the ‘Major Datasets - generated’ field below. All datasets provided need to be publicly available at the point of publication. The author must provide a direct URL to the dataset, not the URL for the entire database. Follow this link and check that you are able to access the dataset. If you need to create a free account in order to view the data then that is okay as it is still technically publicly available.
...
Now navigate to the ‘Changes’ tab. Recorded here is every edit made by the author, Exeter and Crossref. These changes will be automatically approved unless you reject them. When reviewing the changes, you can toggle between viewing the text with and without tracked  changes by clicking on the eye symbol at the top of the window.

![](.gitbook/assets/screenshot-2021-09-08-at-09.20.08.png)

Use the drop down menu to select the role that you would like to filter for. You will need to filter for those made by the content processers, to ensure that no errors have been introduced. Pay attention to cases where the author has reverted or change substantial sections of the copy-editor’s changes to make sure the text still makes sense and so that we are aware of anything that needs feeding back. Author changes need to be checked and accepted one at a time and any mistakes they may have introduced \(stray letters, missing or extra spaces etc\) should be corrected. At this stage, the author should NOT be rewriting large sections of their text. If they have done so, raise the changes with Editorial to check whether the editors need to approve them. Rewording a sentence is OK as long as the content is largely the same. Rewriting the entire discussion is not!

If the author has made edits in the Decision letter or Author response sections, these changes will also need approval from Editorial \(check what I should say here instead\). Check to see if an explanation for the edits has been provided by email or in the notes/response to queries.

Note that any changes you make at this point will also be tracked. However, your name will likely not appear in the list until you refresh the page. Also note that if you navigate away from the Content tab at any point, you will need to refresh before you can accept changes again since moving to another tab causes this tool to stop functioning properly.
...
It can take up to 10 minutes for each of the PDFs to load. Pay particular attention to the figures, key resources table...

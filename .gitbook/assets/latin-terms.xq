...
declare function local:print-specifics($result as element()) as xs:string{
  if ($result/@type="italic") 
    then ('Italicsed terms: '||$result/@count||' ['||string-join(for $item in $result//*:item return ($item/@count||' instance(s) of '||$item),', ')||']')
...
declare function local:print-result($result1 as element(),$result2 as element()) as xs:string{
  ...
  else if (not($roman//*:item) and $italic//*:item) then 'Latin terms are consistently italicsed'
  else if ($roman//*:item and not($italic//*:item)) then 'Latin terms are consistently not italicsed'
  else if (number($roman/@count) = number($italic/@count)) then ('Latin terms are inconsistenly italicsed/roman.&#xa;There are an equal number of roman and italicsed Latin terms '||'('||$roman/@count||')'||'.&#xa;'||local:print-specifics($roman)||'&#xa;'||local:print-specifics($italic)) 
...
let $regex := '[Ii]n\s+[Vv]itro|[Ii]n\s+[Vv]itro|[Ii]n\s+[Vv]ivo|[Ee]x\s+[Vv]ivo|a\s+priori|a\s+posteriori|[Dd]e\s+[Nn]ovo|[Ii]n\s+[Uu]tero|[Ii]n\s+[Nn]atura|[Ii]n\s+[Ss]itu|[Ii]n\s+[Pp]lanta|[Rr]ete\s+[Mm]irabile|[Nn]omen\s+[Nn]ovum| [Ss]ensu |[Aa]d\s+[Ll]ibitum|[Ii]n\s+[Oo]vo'

...
An asset citation is an in-text citation for an [**asset**](allowed-assets/#main-text-assets) that is part of an article. This is in contrast to [**reference citations**](reference-citations.md). These are rendered as links in the final article. Clicking them in the PDF will take the reader to the relevant asset in the PDF and HTML view.

Asset citations may appear inside or outside brackets. Where multiple assets are cited in one set of brackets, each citation should be separated from the next with a comma followed by a space.
...
**Action:** This error will fire if a figure citation appears before a reference e.g. 'See Figure 1 from Smith et al., 1992' \(see more [**here**](allowed-assets/figures.md#figures-that-havent-been-provided)\). This figure citation needs to be changed to plain text. 

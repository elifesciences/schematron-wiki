...
* The supplementary ZIP package. This contains all the other files the authors may have included (figure supplements, video, source data, source code, supplementary files). For PoA, each of these files is assigned a standardised file name based on how they are labeled in the submission system.
* An XML file. This is generated from metadata provided in the submission system, which is currently extracted by querying the database and sending the results to an Amazon Web Service (AWS) bucket as CSV files. The eLife bot scrapes data from these files to populate an XML for each article sent for PoA.
